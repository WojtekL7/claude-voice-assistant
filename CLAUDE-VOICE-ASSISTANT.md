...
- 🔴 **🔍 Szukanie w rozmowie — PODŚWIETLENIE TRAFIENIA PRZESUNIĘTE W LEWO** (zgłoszone i udokumentowane zrzutem 2026-08-03; reszta funkcji POTWIERDZONA, patrz niżej). Szukane „lupa", zaznaczone „wie" z wcześniejszego wyrazu. **Przyczyna ZMIERZONA, nie zgadnięta:** `search_dialog._show_preview` podaje `QTextCursor.setPosition(hit.start)` indeks **PYTHONOWY (punkty kodowe)**, a Qt liczy pozycje w **jednostkach UTF-16** → każde emoji SPOZA BMP przed trafieniem (🤖 U+1F916, 🔍 U+1F50D, ✅…) przesuwa podświetlenie o 1 znak w lewo. Sonda offscreen: tekst z 2 emoji → Qt zaznaczyło `' (lu'` zamiast `'lupa'` (60 vs 62). ⚠️ To NIE jest złamanie zasady „`fold` zachowuje długość" — tamta pilnuje ogonków, ta oś (astralne emoji) była nietknięta; bramki 46/0 przechodzą, bo liczą po stronie Pythona. Kierunek naprawy: przeliczyć `start`/`end` na jednostki UTF-16 (`len(t[:i].encode('utf-16-le'))//2`) TYLKO na styku z Qt + test z emoji przed trafieniem.
