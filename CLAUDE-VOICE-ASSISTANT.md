...
- **Funkcje aplikacji w toku (po kolei):** #1 ✅ własna ikona per zakładka (emoji/plik/paleta-popup). #2 ⏳ kolor zakładki + obramówka na cały program. #3 ⏳ wybór głosu czytającego (**322 darmowe głosy edge-tts**: 2 PL `Marek`/`Zofia`, 47 EN — dropdown z `edge_tts.list_voices()`).
...
3. `gh release download vX.Y.Z -p '*.zip' -p '*.dmg' -p '*.exe'` (do `dist-release/`, w .gitignore). ⚠️ Wzorzec **`*Setup.exe` NIE łapie** `ClaudeVoiceAssistant-Setup-1.0.20.exe` (kończy się na `-X.Y.Z.exe`, nie na `Setup.exe`) — używaj `*.exe`.
...
## Inne otwarte TODO
- [ ] **1.0.24 — POTWIERDZIĆ u usera (priorytet)**: test AppImage w `/cva/ClaudeVoiceAssistant-1.0.24-linux-x64.AppImage` (NIE w appcaście). Sprawdzić: zaznaczanie bez Shift + kopiowanie + 🔊 + czytanie F-P + flaga „?". **Obserwacja:** PRZEWIJANIE w Claude (B1 zabiera mysz) — jeśli przeszkadza → zrobić PRZEŁĄCZNIK „tryb zaznaczania" zamiast always-on. Kod scommitowany na main; **wydanie (build Mac/Win + podbicie appcastu) dopiero po „działa"**.
...
- [ ] **Funkcja #2 (NASTĘPNA): kolor zakładki + obramówka na cały program** — per-zakładka kolor, który koloruje też ramkę/obwódkę programu.
- [ ] **Funkcja #3: wybór głosu czytającego** — dropdown z `edge_tts.list_voices()` (322 darmowe; 2 PL, 47 EN), per-agent lub globalnie.
