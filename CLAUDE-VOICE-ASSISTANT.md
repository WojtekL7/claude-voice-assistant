...
## AKTUALNY STAN (wersja 1.0.20)
- **1.0.20 (wydane, pipeline e2e OK):** fix białego błysku terminala (patrz pułapki) + kreator ustawień przebudowany na **listę kontrolną 3 punktów** (Claude zainstalowany/zalogowany/dyktowanie; pokazywany tylko gdy czegoś brakuje). **W repo, ale NIEWYDANE (pojedzie z 1.0.21):** scalenie instrukcji macOS/Linux/Windows w jedną stronę z górnym menu + `config.install_guide_url` linkuje do niej. Wydanie 1.0.21 odłożone (czeka aż nazbiera się więcej).
