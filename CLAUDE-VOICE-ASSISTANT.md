...
- **🤖 „Domyślny (Opus 5)" na pasku** (2026-07-27, `7c3e264`) — przy ustawieniu agenta „Domyślny" pasek dopisuje model **WYKRYTY z dziennika** (`transcript_reader.active_model()` → `config.model_name_for_api_id`), a licznik tokenów bierze okno TEGO modelu zamiast założonego 1 mln. ⚠️ **Napis opisuje stan Z DZIENNIKA, nie ustawienie agenta** — w `agents.json` dalej stoi `model: "default"` (VCA/CRM/AS/F-P.pl); przy następnej diagnozie nie czytaj z paska, że agent ma przypięty Opus. Model łapie `poll()` za darmo; skan ogona (256 KB, próg 10 s) tylko gdy poll nic nie ma — po restarcie apki przy trwającej rozmowie. Nieznany model → nazwa wyprowadzona z identyfikatora, nigdy zmyślona; brak danych → samo „Domyślny". Bramka `tools/test-detected-model.py` **20/0** (nagłówek pliku niesie WYNIKI SABOTAŻU — dwa pierwsze testy nie rozróżniały). Sprawdzone na 6 żywych zakładkach + render offscreen PL/EN; **GUI na żywo NIE** (wymaga restartu apki). Test: pasek VCA/CRM pokazuje „Domyślny (Opus 5)" · tooltip „Aktualnie odpowiada: …" · SEO Manager bez zmian („Opus 5") · nowa zakładka uzupełnia nazwę po 1. odpowiedzi.
...
- **🆕 KATALOG MODELI** (2026-07-26) — lista pokazuje teraz Opus 5 / Sonnet 5, doszedł przypięty `claude-opus-4-8`, a nazwy odświeżają się ze strony Anthropic. Test: (1) Ustawienia → agent → lista modeli pokazuje „Opus 5 (najbardziej zdolny)"; (2) Ustawienia → „Sprawdź nowe modele" → komunikat „Lista modeli jest aktualna"; (3) po odcięciu internetu ta sama pozycja mówi, że nic się nie zepsuło, a lista zostaje; (4) pasek statusu pokazuje „🤖 Opus 5"; (5) wybór „Opus 4.8" startuje zakładkę normalnie. → sekcja „MODELE" niżej.
