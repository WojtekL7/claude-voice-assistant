...
- [ ] 🔴 **BUG Mac — CRASH QtWebEngine po wybudzeniu — TRZECIA WERSJA Z RZĘDU, warunek „gdyby się nasiliło" SPEŁNIONY.** Wystąpienia: 1.0.25 (`Time Since Wake` **88 s**) · 1.0.26 (raz) · **1.0.28 (2026-08-07 17:42, Time Since Wake 142 s, macOS 13.4 ARM-64)**. Sygnatura stała: `Crashed Thread 0 CrBrowserMain`, `EXC_BAD_ACCESS (SIGSEGV)`, adres **spoza jakiegokolwiek obszaru VM** (`KERN_INVALID_ADDRESS`) = wskaźnik na zwolniony obiekt. Nowe ze zrzutu 1.0.28: szczyt stosu to **`QWidget::isActiveWindow()` → `QWidget::palette()`** (QtWidgets), czyli wątek przeglądarki dotyka WIDŻETU, którego już nie ma — to trop mocniejszy niż samo „QtWebEngine się wysypał".
  - ⚠️ **NIE jest to regresja żadnego wydania** — sygnatura identyczna na trzech różnych wersjach; wyzwalaczem jest WYBUDZENIE, nie kod wydania. Rozpoznanie w 1 strzał: **`Time Since Wake` liczone w sekundach/minutach** (nie w godzinach).
  - Apka wstaje po „Otwórz ponownie" i awaria nie wraca do następnego uśpienia → dla usera uciążliwość, nie utrata pracy.
  - ⚠️ Nasz `crash-logs/` łapie TYLKO crash `claude`, NIE QtWebEngine → jedynym źródłem jest raport Apple; do domknięcia potrzebny PEŁNY stos (na zrzucie widać tylko ramki 0–2, reszta ucięta).
  - Kierunek naprawy (do decyzji usera): flagi QtWebEngine na macOS albo reinicjalizacja WebTerminala na sygnał wybudzenia. → `mac-qtwebengine-crash-po-snie.md`
