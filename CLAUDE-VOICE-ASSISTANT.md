...
- 🔬 **🔊 „czytaj ostatnią" — RUNDA 4 = SAM POMIAR, zero zmian zachowania** (`d71e827`, 2026-08-03, NIEPRZETESTOWANE u usera — wymaga restartu bety). Czujnik jest **już włączony** plikiem-znacznikiem `~/.vibe-coding-assistant/read-last-debug.on`; czekamy, aż user napisze **„źle"** po złym odczycie (czas kliknięcia biorę z timestampu JEGO wiadomości w dzienniku mojej sesji).
  - 🔴 **Odkrycie: przez trzy rundy naprawialiśmy NA ŚLEPO** — `read-last-debug.log` u usera NIE ISTNIAŁ. Czujnik włączała tylko zmienna `CVA_READ_LAST_DEBUG=1`, a apkę uruchamia się ikoną z pulpitu (rodzic = `gnome-shell`). ⛔ **Diagnostyka na zmiennej środowiskowej jest dla nietechnicznego usera MARTWA** — dlatego plik-znacznik.
  - 🔴 **Odkrycie: gałąź ZAZNACZENIA nie miała żadnego śladu w logu**, a beta usera chodzi na **WebTerminalu** (`CVA_WEBTERMINAL=1` w `/proc/PID/environ`) — tam zaznaczenie jest lepkie i przez `SELECTION_FRESH_SECS=30 s` CELOWO wypiera dziennik. Objaw byłby nieodróżnialny od „czyta przedostatnią", a żadna z 3 rund tego nie dotykała. Milczały też gałąź EKRANU i to, co realnie poszło do lektora.
  - Każdy klik zapisuje teraz: zakładkę · silnik · wiek zaznaczenia · OSTATNIĄ i PRZEDOSTATNIĄ wypowiedź · jawne `ŹRÓDŁO=ZAZNACZENIE/DZIENNIK/EKRAN` · treść do lektora. Sonda: komplet 9 pól + kontrola negatywna (zgaszony czujnik nie pisze).
