...
- 🔊 **TEMPO CZYTANIA — przycisk 1× / 1,25× / 1,5× / 2× na dolnym pasku** (`720339d`, 2026-09-17, **NIEPRZETESTOWANE u usera** — beta wstała 09:44, poprawka powstała po południu; właściciel testuje 18.09 „podczas pracy"). Prośba właściciela; świadoma decyzja: **BEZ spowolnienia 0,5×**, cykl startuje od 1×.
  ...
  - **Dowody:** `tools/test-tts-speed.py` **24/24** (w tym asercja na ŁADUNKU WYCHODZĄCYM — tempo realnie dojeżdża do `edge_tts.Communicate`) · `tools/sabotaz-tts-speed.py` **14 wariantów, każdy wykryty**, 24 wykonane sprawdzenia w KAŻDYM przebiegu, każdy plik przywrócony co do `sha256` · regresja **28/28 bramek zielonych**, w tym `test-bottom-bar-icons` 48/48 · podgląd offscreen: tło nowego przycisku **35**, identyczne z sąsiadami; „1,25×" = 36 px w przycisku 52 px.
  - ⚠️ **NAUKA O WŁASNEJ BRAMCE, warta przeczytania przed pisaniem następnej:** sabotaż S2 **za pierwszym razem nie wykrył NICZEGO — i winna była BRAMKA, nie kod.** Asercja pytała, czy w pliku STOI napis o stylowaniu, a sabotaż zamieniał warunek na `if False:`, zostawiając tę linię w źródle. Po przepisaniu na **pomiar SKUTKU** (wywołanie prawdziwej `_apply_button_icon_styles` na atrapie + odczyt `styleSheet()`) ten sam sabotaż zapala dokładnie jedną asercję. ⭐ Osobno S5 dowiódł, że para asercji D1/D2 to NIE duplikaty: podstawienie stałej `"+0%"` zapala D1, ale NIE D2, bo `+0%` nadal jest na liście dozwolonych. **Nie kasuj żadnej jako nadmiarowej.**
  - **DO TESTU PO RESTARCIE (7 punktów, pełna lista w opisie commita `720339d`):** (a) cykl 1×→1,25×→1,5×→2×→1×; (b) na 2× szybciej **bez zmiany wysokości głosu**; (c) kontrola odwrotna — 1× brzmi jak dotąd; (d) tempo przeżywa zamknięcie programu; (e) ta sama wartość we wszystkich zakładkach; (f) regresja pauzy, stopu i auto-czytania; (g) język EN → napis „1.25×" z kropką.
