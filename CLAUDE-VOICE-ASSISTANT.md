...
## Inne otwarte TODO
- [ ] **Flaga „?" — fałszywy negatyw (czeka na realny przypadek):** user zgłasza „raz działa, raz nie" = flaga nie pojawia się, choć agent czeka. W `main_window._poll_transcripts` jest **TYMCZASOWA diagnostyka** (log `~/.claude-voice-assistant/flag-debug.log`, zapis przy zmianie stanu) — **NIEZACOMMITOWANA** (tylko working tree `main_window.py`). Gdy user złapie niezadziałanie, odczytać log → który warunek zawiódł (`journal_quiet` czy `terminal_quiet`) → celna naprawa, potem USUNĄĆ diagnostykę. Podejrzenie: przerysowujący się ekran „czekam" trzyma `terminal_quiet=False`.
- [ ] **Opublikować 1.0.22 dla Mac/Linux + appcast** — gdy 1.0.22 Windows potwierdzona; zbudować mac (CI) + linux (lokalnie), wgrać 3 paczki, DOPIERO POTEM podbić appcast (globalna version — patrz pamięć `wydanie-do-poczekalni-appcast`).
